...
AI/CV-сервис не входит в этот пакет. Его runtime, модели, установщик, обновление
и rollback принадлежат отдельному репозиторию
[`bag-counter-cv-service`](https://github.com/arystambek-dimash/bag-counter-cv-service).
Устанавливать или восстанавливать AI нужно только по runbook того репозитория;
копировать реализацию из `asyl-ltd` на Windows-ПК нельзя. Перед миграцией любого
другого установленного CV-пакета отдельно сверяйте совместимость его state и
SQLite journal — эти данные нельзя заменять пустыми или переносить между
несовместимыми схемами автоматически.

Прямые substream-адреса камер для AI также настраиваются только актуальным
camera-PC пакетом из `bag-counter-cv-service` в защищённом
`C:\ProgramData\ASYL-Camera-Agent\direct-substreams.json`. Этот пакет Asyl LTD
не знает RTSP-паролей и не должен создавать такой файл. Назначение `camN` в
Моноблоке меняет серверную политику AI 24/7; использовать физическую камеру
сервис сможет лишь после того, как оператор применил и проверил соответствующий
прямой `camNsub` по runbook CV-репозитория.

Внешний CV-сервис публикует аннотированные потоки `cam…ai`, а этот пакет только
настраивает их запись и playback в MediaMTX.

Перед совместной выкладкой нового CV-контракта и Asyl LTD обязателен maintenance
barrier: временно запретить новые старты Моноблока, завершить или отменить все
открытые `AiCountingSession` и активные привязки `loading_camera`, и проверить,
что граница сессий пуста. Только после этого обновить CV, проверить `camNsub`,
обновить Asyl LTD и снять запрет. CV-first безопасен только на пустой границе:
старый Asyl способен открыть на новом CV сессию без durable `session_id`, которую
новый backend не имеет права завершать как точную сессию заказа.
