...
# запросами (рестарт Postgres, простой).
#
# Django держит соединение на поток, поэтому потолок — это
# workers × threads (3 × 8 = 24, см. Dockerfile) плюс по одному на каждый
# долгоживущий монитор: около 26 при дефолтных 100 у Postgres. Увеличивая
# воркеры или потоки, пересчитайте это против max_connections.
DATABASES = {
    "default": {
