...
from apps.common import daemon
...
def test_initial_heartbeat_is_published_before_first_poll(heartbeat_path):
    ...
    with (
        patch.object(daemon, "close_old_connections"),
        patch.object(
            scheduling,
            "poll_once",
        ...
        call_command("monitor_shipping_sessions", "--once", stdout=StringIO())
...
def test_loop_recovers_from_dependency_failure_and_stops_after_active_poll(
    ...
    with (
        patch.object(daemon, "close_old_connections"),
        patch.object(daemon.threading, "Event", return_value=stopped),
        patch.object(stopped, "wait", side_effect=waits.append),
        patch.object(daemon.signal, "signal", side_effect=handle_signal),
        patch.object(daemon.time, "monotonic", side_effect=[0, 1, 2, 3]),
        patch.object(
            scheduling, "ShippingSessionScheduler", return_value=scheduler
        ) as scheduler_class,
        patch.object(scheduling, "poll_once") as synchronous_poll,
    ):
        call_command("monitor_shipping_sessions", stdout=StringIO())
...
def test_programming_error_exits_for_supervisor_restart(heartbeat_path):
    with (
        patch.object(daemon, "close_old_connections"),
        patch.object(
            scheduling, "poll_once", side_effect=ValueError("bug")
        ),
...
def test_command_rejects_invalid_poll_interval(interval):
    with pytest.raises(CommandError, match="between 0.5 and 60"):
        call_command("monitor_shipping_sessions", "--once", "--interval", interval)
