...
from apps.cameras import production_queries, production_runs
...
def test_stale_cleanup_skips_busy_run_and_preserves_concurrent_refresh():
    ...
    runs = [
        AlwaysOnProductionRun.objects.create(
            camera=camera,
            color="red",
            business_day=production_runs.business_day_for(now),
            started_at=old,
            last_counted_at=old,
            model_bags=1,
        )
    ...
    def cleanup():
        try:
            return production_runs.close_stale_runs(now)
        finally:
...
def test_reading_production_projects_stale_status_without_mutating_any_camera():
    ...
    day = timezone.localtime(rows[0].last_counted_at).date()
    with CaptureQueriesContext(connection) as queries:
        result = production_queries.production_payload("cam3", day=day)
    for row in rows:
        row.refresh_from_db()
        assert row.ended_at is None
    assert result["day_runs"][0]["status"] == "closed"
    assert result["day_runs"][0]["ended_at"] is not None
    assert all(
...
def test_production_batch_query_count_does_not_grow_with_history():
    ...
    def add_batch(offset):
        batch = AlwaysOnStockBatch.objects.create(
            ...
            scheduled_for=production_runs.scheduled_for(day - timedelta(days=offset)),
    ...
    with CaptureQueriesContext(connection) as first:
        production_queries.production_payload("cam3")
    ...
    assert len(full) == len(first), f"1 batch={len(first)}, 31 batches={len(full)}"
