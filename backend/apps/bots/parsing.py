...
def format_rail_report(
    *, day: date, country: str, client_name: str, station: str, wagons: Iterable[tuple[str, str, Decimal]],
    ...
    ``wagons`` — (код товара, номер вагона, тонны). «Отправить отчёт» в
    истории грузчика (:mod:`apps.bots.wagon_report`): текст отгрузки по отчёту
    разбирается обратно в те же вагоны. Страна не из списка шапки опускается —
    иначе разбор принял бы её за клиента.
    """
    wagons = list(wagons)
