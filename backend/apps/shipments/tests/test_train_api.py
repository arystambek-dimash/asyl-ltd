...
def loader(user_with_perms):
    return user_with_perms(
        "loader",
        codes=["monoblock.view", "loader.view", "loader.confirm", "loader.wagons"],
    )
...
def test_train_action_flow(loader, boss):
    ...
    assert (
        api.post(
            f"/api/orders/{o.id}/train/", {"action": "count", "bags": 10}, format="json"
        ).status_code
        == 200
    )
...
def test_train_bad_action_400(loader, boss):
    o = _train_order(boss)
    r = _api(loader).post(
        f"/api/orders/{o.id}/train/", {"action": "nope"}, format="json"
    )
...
def test_train_count_rejects_invalid_bags(loader, boss, bags):
    ...
    response = api.post(
        f"/api/orders/{o.id}/train/", {"action": "count", "bags": bags}, format="json"
    )
