"""Человекочитаемые подписи заказа и оплаты — единый словарь на весь бэкенд.

Раньше метки способов оплаты жили в трёх местах и разошлись: кассир видел
«Наличные» в интерфейсе, «Наличными» в Excel-выписке и «Kaspi Pay» вместо
«Kaspi» в PDF-квитанции. То же было с транспортом — «Поезд» и «Вагон» в одном
файле. Здесь подпись задаётся один раз; вариант для архивных выписок
запрашивается флагом, а не отдельной копией словаря.
"""
...
# Способы, которыми больше нельзя создать оплату: в исторических документах
# помечаем явно, чтобы бухгалтер не искал их в кассе.
