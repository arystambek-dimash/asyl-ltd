...
def test_list_filters_orders_by_inclusive_creation_date_range(manager, api_as):
...
def test_list_rejects_invalid_date_filters(manager, query, api_as):
    response = api_as(manager).get(f"/api/orders/?{query}")
    assert response.status_code == 400


def test_list_filters_by_public_status_group(manager, api_as):
    ...
    r = api_as(manager).get("/api/orders/?status_group=confirmed")
    assert r.status_code == 200
    assert {row["id"] for row in r.data} == {
        statuses["confirmed"].id, statuses["arrived"].id, statuses["loading"].id}

    r = api_as(manager).get("/api/orders/?status_group=loaded")
    assert {row["id"] for row in r.data} == {statuses["loaded"].id}

    r = api_as(manager).get("/api/orders/?status_group=shipped")
    assert {row["id"] for row in r.data} == {statuses["shipped"].id}

    assert api_as(manager).get("/api/orders/?status_group=bogus").status_code == 400
