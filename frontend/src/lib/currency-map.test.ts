...
describe("currency maps", () => {
  it("keeps currencies separate and does not fall back from a populated map", () => {
    ...
    expect(amountForCurrency(totals, "KZT")).toBe(1000);
    expect(amountForCurrency(totals, "EUR")).toBe(0);
    expect(amountForCurrency({}, "KZT")).toBe(0);
  ...
  it("ignores zero currencies and defaults to tenge", () => {
  ...
  it("picks one field of a per-currency breakdown", () => {
    const summary = { KZT: { paid: "100", debt: "5" }, USD: { paid: "2", debt: "0" } };
    expect(fieldByCurrency(summary, "paid")).toEqual({ KZT: "100", USD: "2" });
  });
});
