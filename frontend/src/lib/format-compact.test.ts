...
describe("formatCompact", () => {
  ...
  it("сокращает тысячи", () => {
    expect(formatCompact(100_000)).toBe(nb("100 тыс"));
    expect(formatCompact(993_045)).toBe(nb("993 тыс"));
  });

  it("сокращает миллионы", () => {
    expect(formatCompact(1_000_000)).toBe(nb("1 млн"));
    expect(formatCompact(9_930_456)).toBe(nb("9,93 млн"));
    expect(formatCompact(111_026_971)).toBe(nb("111 млн"));
  });

  it("сокращает миллиарды", () => {
    expect(formatCompact(4_809_747_848.94)).toBe(nb("4,81 млрд"));
    expect(formatCompact(4_860_447_427.96)).toBe(nb("4,86 млрд"));
  });

  it("держит знак у отрицательных", () => {
    expect(formatCompact(-11_079)).toBe(nb("-11 079"));
    expect(formatCompact(-4_295_732_000)).toBe(nb("-4,3 млрд"));
  });

  it("принимает строку — суммы приходят с бэкенда строками", () => {
    expect(formatCompact("10077481.97")).toBe(nb("10,1 млн"));
  });
...
describe("formatCompactCurrency", () => {
  it("подставляет символ валюты", () => {
    expect(formatCompactCurrency(4_809_747_848.94)).toBe(`${nb("4,81 млрд")} ₸`);
    expect(formatCompactCurrency(1_500_000, "USD")).toBe(`${nb("1,5 млн")} $`);
  });
});
