...
function EmployeesPageInner() {
  ...
  return (
    <AppShell
      title="Сотрудники"
      section="Управление"
      description="Учётные записи сотрудников и их роли. Создавайте аккаунты и назначайте доступ."
      actions={
        canManage ? (
            ...
            <TBody>
              {sorted.map((e) => (
                <TR key={e.id}>
                  ...
                  <TD>
                    <div>{e.role_name || "—"}</div>
                    <div className="text-xs text-[var(--muted-foreground)]">Доступов: {effectiveAccessCount(e)}</div>
                  </TD>
                  <TD>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-[var(--muted-foreground)]">
                    {rolePerms.size > 0 ? `Из роли: ${rolePerms.size - deniedCodes.size} · ` : ""}Личных: {codes.size}
