...
function RolesPageInner() {
  ...
  function openEdit(r: Role) {
    setEditing(r);
    setName(r.name);
    setCodes(new Set(r.permissions.map((p) => p.code)));
    setError("");
    setOpen(true);
  }
  ...
  async function confirmRemove() {
    ...
    setDelError("");
    try {
      await api.delete(`/roles/${delRole.id}/`);
      setDelRole(null);
      reload();
    } catch (e) {
          ...
          <Card key={r.id}>
            <CardHeader className="flex-row items-center justify-between">
              <CardTitle className="flex items-center gap-2">
                {r.name}
                {r.is_system && <Badge tone="muted">системная</Badge>}
              </CardTitle>
            </CardHeader>
            <CardContent className="flex flex-col gap-3">
              <p className="text-xs text-[var(--muted-foreground)]">
                Прав: {r.permissions.length} · Сотрудников: {r.employee_count}
              </p>
      ...
      >
        <form onSubmit={save} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-4">
            <Label id="role-permissions-label">Права роли — действуют на всех сотрудников с этой ролью</Label>
            <div role="group" aria-labelledby="role-permissions-label">
