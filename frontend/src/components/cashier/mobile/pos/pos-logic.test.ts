...
describe("posReducer", () => {
  ...
  it("keeps the selection when switching payment modes before issuing", () => {
    ...
    expect(posReducer(phone, { type: "tab", tab: "qr" })).toMatchObject({ tab: "qr", flow: "qr", step: "amount" });
    expect(posReducer(phone, { type: "tab", tab: "remote" })).toBe(phone);
  });

  it("applies the URL tab on entry only while no payment is in progress", () => {
    expect(posReducer(INITIAL_POS_STATE, { type: "enter", tab: "remote" })).toMatchObject({
      tab: "remote",
      flow: "remote",
    });
    expect(posReducer(picked, { type: "enter", tab: "remote" })).toBe(picked);
    const issued = posReducer(picked, { type: "issued", payment: payment() });
    expect(posReducer(issued, { type: "enter", tab: "qr" })).toBe(issued);
  });
