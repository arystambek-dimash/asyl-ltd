 ...
 */
export function FilterDropdown({ label, options, active, onChange }: {
  label: string;
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button type="button" onClick={() => setOpen((o) => !o)}
        ...
        <ChevronDown className={cn("size-3.5 text-[var(--muted-foreground)] transition-transform", open && "rotate-180")} />
      </button>
      ...
      {open && (
        <div role="listbox"
          ...
          {options.map((o) => {
            ...
            return (
              <button key={o.key} type="button" role="option" aria-selected={on}
